...
//                && StringUtils.endsWithIgnoreCase(httpServletRequest.getMethod(),"post")){
    ...
    private void validateCode(ServletWebRequest servletWebRequest) throws ValidateCodeException, ServletRequestBindingException {
        ...
        //从session中获取验证码
        ImageCode codeInSession = (ImageCode) sessionStrategy.getAttribute(servletWebRequest, ValidateCodeController.SESSION_VERIFY_CODE_KEY);
        ...
        if (codeInSession.isExpried()) {//过期之后，删除验证码
            sessionStrategy.removeAttribute(servletWebRequest, ValidateCodeController.SESSION_VERIFY_CODE_KEY);
